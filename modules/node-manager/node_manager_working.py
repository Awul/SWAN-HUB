...
table_dirty = True
STATUS_TOPIC = "swan-hub/nodes/status"  # single topic for all node info
...
def publish_status(client):
    """Publish all current nodes + sensors as a retained message"""
    payload = json.dumps({
        node_id: {
    ...
    client.publish(STATUS_TOPIC, payload=payload, retain=True)
    log.debug(f"Published node status to {STATUS_TOPIC}")
...
def print_table():
    ...
    for node_id, node in nodes.items():
        ...
        sensors = " ".join(f"{k}={v}" for k, v in node["sensors"].items())
...
def on_message(client, userdata, msg):
    ...
    if len(topic) < 4:
        return
    node_id, subtopic = topic[2], topic[3]
    ...
    if subtopic == "heartbeat":
        try:
            ...
            log.info(f"Heartbeat from {node_id} │ fw={data.get('firmware')} │ uptime={data.get('uptime')}s │ sensors={sensors}")
            table_dirty = True
        except Exception:
    ...
    # -----------------------
        ...
        try:
            ...
            value = payload
        if node_id not in nodes:
            nodes[node_id] = {"last_seen": now(), "firmware": None, "uptime": None, "sensors": {}}
            log.info(f"Node discovered: {node_id}")
        nodes[node_id]["last_seen"] = now()
        old = nodes[node_id]["sensors"].get(sensor)
        if old != value:
            nodes[node_id]["sensors"][sensor] = value
            log.info(f"Sensor update {node_id}/{sensor} = {value}")
            table_dirty = True
